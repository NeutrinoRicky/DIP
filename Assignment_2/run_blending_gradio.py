...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    fg_laplacian = torch.nn.functional.conv2d(foreground_img, kernel, padding=1, groups=foreground_img.shape[1])
    blended_laplacian = torch.nn.functional.conv2d(blended_img, kernel, padding=1, groups=blended_img.shape[1])

    fg_valid = (foreground_mask > 0.5).expand(-1, foreground_img.shape[1], -1, -1)
    bg_valid = (background_mask > 0.5).expand(-1, blended_img.shape[1], -1, -1)

    fg_values = fg_laplacian[fg_valid]
    bg_values = blended_laplacian[bg_valid]

    valid_num = min(fg_values.numel(), bg_values.numel())
    if valid_num > 0:
        loss = torch.nn.functional.mse_loss(bg_values[:valid_num], fg_values[:valid_num])
