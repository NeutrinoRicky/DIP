...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        J_proj = torch.zeros(
            (N, 2, 3), device=means3D.device, dtype=means3D.dtype
        )
        fx, fy = K[0, 0], K[1, 1]
        x, y = cam_points[:, 0], cam_points[:, 1]
        inv_z = depths.reciprocal()
        inv_z_sq = inv_z.square()
        J_proj[:, 0, 0] = fx * inv_z
        J_proj[:, 0, 2] = -fx * x * inv_z_sq
        J_proj[:, 1, 1] = fy * inv_z
        J_proj[:, 1, 2] = -fy * y * inv_z_sq
        
        # Transform covariance to camera space
        R_batch = R.unsqueeze(0).expand(N, -1, -1)
        covs_cam = torch.bmm(
            R_batch, torch.bmm(covs3d, R_batch.transpose(1, 2))
        )  # (N, 3, 3)
    ...
    def compute_gaussian_values(
        ...
        # positive definite after the diagonal regularization above.
        # Adding eps to both eigenvalues permits determinants as small as
        # eps**2, so use that (rather than eps) as the numerical floor.
        det = torch.linalg.det(covs2D).clamp_min(eps ** 2)
        inv_covs2D = torch.linalg.inv(covs2D)
        mahalanobis_sq = torch.einsum(
            'nhwi,nij,nhwj->nhw', dx, inv_covs2D, dx
        )
        normalizer = 2.0 * torch.pi * torch.sqrt(det)
        gaussian = torch.exp(-0.5 * mahalanobis_sq) / normalizer[:, None, None]
    ...
    def forward(
        ...
        # 7. Compute weights
        # Exclusive cumulative product gives T_i = product_{j<i}(1-alpha_j).
